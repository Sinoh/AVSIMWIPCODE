...
main (int argc, char *argv[])
{
  ...
  // static const ns3::InetSocketAddress kBeaconBroadcast = ns3::InetSocketAddress(ns3::Ipv4Address("255.255.255.255"), 4001);
  ...
  Ptr<TraciClient> AirSimClient = CreateObject<TraciClient> ();
  AirSimClient->SetAttribute ("SynchInterval", TimeValue (Seconds (10.0)));
  AirSimClient->SetAttribute ("StartTime", TimeValue (Seconds (0.0)));
  AirSimClient->SetAttribute ("AirSimPort", UintegerValue (4001));
  AirSimClient->SetAttribute ("AirSimWaitForSocket", TimeValue (Seconds (5.0))); // AirSimCntrlwait for socket
  // AirSimClient->SetAttribute ("AirSimWaitForSocket", TimeValue (Seconds (5.0)));
  // TODO: Add AirSimCar sockets
  ...
  rsuSpeedControlHelper.SetAttribute ("Client", (PointerValue) (AirSimClient));    // pass TraciClient object for accessing sumo in application
  ...
  vehicleSpeedControlHelper.SetAttribute ("Client", (PointerValue) AirSimClient); // pass TraciClient object for accessing sumo in application
  ...
  // AirSimClient->SumoSetup (setupNewWifiNode, shutdownWifiNode);
  // used for Fake Clients
  AirSimClient->airSimSetup (setupNewWifiNode, shutdownWifiNode);
