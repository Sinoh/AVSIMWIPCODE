...
namespace tcpip
{
		...
		const
	{
      ...
      std::cout << "\n bytes rec" << bytesReceived << " expecting " << len;

			len -= bytesReceived;
			buffer += bytesReceived;
		}
    std::cout << "leaving receiveComplete\n";
	}
    ...
    std::cout << "in recieveExact! with " << lengthLen;
    ...
    std::cout << "go back in to recieve complete if >0 : " << totalLen - lengthLen;
  ...
  bool
    ...
    std::cout << "in testRecieveExact!\n";

		// receive length of TraCI message
    const size_t bytesReceived = recvAndCheck(&buffer[0], 2048);
    std::cout << "bytes rec " << bytesReceived << "\n";
    ...
    std::cout << "\n";
		...
		// receiveComplete(&buffer[0], buffer.size());
		// Storage length_storage(&buffer[0], bytesReceived);
		// const int totalLen = length_storage.readInt();
		// assert(totalLen > lengthLen);
    ...
    // std::cout << "go back in to recieve complete if >0 : " << totalLen - lengthLen;
		...
		// copy message content into passed Storage
		msg.reset();
		msg.writePacket(&buffer[0], bytesReceived);
		
		printBufferOnVerbose(buffer, "Rcvd Storage with");

    std::cout << "Leaving testReceiveExact\n";
